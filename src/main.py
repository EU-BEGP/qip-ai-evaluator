...
def load_or_extract_criteria(input_file):
    """Step 2: Load criteria from Excel file or extract from CSV."""
    print("\n=== Step 2: Load or Extract Criteria ===")
    logging.info("Loading or extracting evaluation criteria...")
    cfg = load_config()
    scans_path = resolve_project_path(
        cfg.get("evaluation", {}).get("scans_csv_path"),
        "data/scans/scans.csv"
    )
    scans_path.parent.mkdir(parents=True, exist_ok=True)

    if input_file.lower().endswith(".csv"):
        shutil.copy(input_file, scans_path)
        logging.info(f"Criteria CSV loaded and saved to {scans_path}")
    elif input_file.lower().endswith((".xlsx")):
        extractor = CSVCriteriaExtractor(input_file, str(scans_path))
        extractor.process_file()
        logging.info(f"Criteria extracted from {input_file}")
    else:
...
def main():
    ...
    # Step 2: Load or Extract criteria
    criteria_file = input("Enter criteria file path (.xlsx or .csv): ").strip()
